...
class TSOSK_Mod_Recovery {
	...
	public function ajax_resume(): void {
		...
		if ( '' === $extension || ! in_array( $type, array( 'plugin', 'theme' ), true ) ) {
			wp_send_json_error( __( 'Invalid recovery item.', 'tso-swiss-knife' ) );
		}
		...
		if ( ! is_object( $storage ) || ! method_exists( $storage, 'delete' ) ) {
			wp_send_json_error( __( 'Recovery storage is not available on this WordPress version.', 'tso-swiss-knife' ) );
		}

		$storage->delete( $extension );
		wp_send_json_success( __( 'The paused extension entry was cleared. Reload and test carefully.', 'tso-swiss-knife' ) );
	}
	...
	public function render(): void {
		...
		$handler_label = is_object( $fatal_handler ) ? get_class( $fatal_handler ) : __( 'Not available', 'tso-swiss-knife' );
		?>
		<p class="tsosk-desc">
			<?php esc_html_e( 'WordPress Recovery Mode allows an admin to log in and deactivate plugins or themes that cause a fatal error.', 'tso-swiss-knife' ); ?>
		</p>

		<div class="tsosk-card">
			<h3><?php esc_html_e( 'What you can do here', 'tso-swiss-knife' ); ?></h3>
			<p><?php esc_html_e( 'Recovery Mode is a WordPress safety feature. It is not a permanent on/off switch: WordPress enters it when a fatal error is detected, or when you open a valid recovery URL.', 'tso-swiss-knife' ); ?></p>
			<ul>
				<li><?php esc_html_e( 'Generate and open a one-time recovery URL.', 'tso-swiss-knife' ); ?></li>
				<li><?php esc_html_e( 'See plugins or themes that WordPress has paused after a fatal error.', 'tso-swiss-knife' ); ?></li>
				<li><?php esc_html_e( 'Clear a paused item after you have fixed the error, so WordPress can try loading it again.', 'tso-swiss-knife' ); ?></li>
			</ul>
		</div>

		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Recovery Mode Status', 'tso-swiss-knife' ); ?></h3>
			<table class="tsosk-kv-table">
				<tr>
					<th><?php esc_html_e( 'Currently Active', 'tso-swiss-knife' ); ?></th>
					<td><?php echo $is_active
						? '<span class="tsosk-badge tsosk-badge-warn">' . esc_html__( 'YES – Recovery Mode is active', 'tso-swiss-knife' ) . '</span>'
						: '<span class="tsosk-badge tsosk-badge-ok">'  . esc_html__( 'No', 'tso-swiss-knife' ) . '</span>'; ?></td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Recovery Email', 'tso-swiss-knife' ); ?></th>
					<td><code><?php echo esc_html( get_option( 'admin_email' ) ); ?></code></td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Fatal Error Handler', 'tso-swiss-knife' ); ?></th>
		...
		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Recovery Mode URL', 'tso-swiss-knife' ); ?></h3>
			<p><?php esc_html_e( 'Use this URL to enter recovery mode. It is single-use and expires after 1 hour.', 'tso-swiss-knife' ); ?></p>
			...
			<p><a href="<?php echo esc_url( $recovery_url ); ?>" class="button button-primary">
				<?php esc_html_e( 'Enter Recovery Mode', 'tso-swiss-knife' ); ?>
			</a></p>
		</div>
		...
		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Error Protection Log', 'tso-swiss-knife' ); ?></h3>
			<?php
			...
			if ( empty( $paused_plugins ) && empty( $paused_themes ) ) {
				echo '<p>' . esc_html__( 'No plugins or themes have been auto-paused by WordPress.', 'tso-swiss-knife' ) . '</p>';
			} else {
				if ( ! empty( $paused_plugins ) ) {
					echo '<h4>' . esc_html__( 'Paused Plugins', 'tso-swiss-knife' ) . '</h4><ul class="tsosk-recovery-list">';
					foreach ( $paused_plugins as $plugin => $data ) {
						echo '<li><strong>' . esc_html( $plugin ) . '</strong>: '
						     . esc_html( $data['message'] ?? __( 'Unknown error', 'tso-swiss-knife' ) )
						     ...
						     . esc_html__( 'Clear pause', 'tso-swiss-knife' )
						     . '</button></li>';
					}
					echo '</ul>';
				}
				if ( ! empty( $paused_themes ) ) {
					echo '<h4>' . esc_html__( 'Paused Themes', 'tso-swiss-knife' ) . '</h4><ul class="tsosk-recovery-list">';
