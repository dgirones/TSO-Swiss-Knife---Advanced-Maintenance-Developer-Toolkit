...
class TSOSK_Mod_Roles {
	...
	public function ajax_caps(): void {
		...
		if ( null === $caps ) {
			wp_send_json_error( __( 'Role not found.', 'tso-swiss-knife' ) );
		}
	...
	private function cap_description( string $cap ): string {
		$map = array(
			'level_0'           => __( 'Legacy user level 0 (subscriber). Deprecated — ignore.', 'tso-swiss-knife' ),
			'level_1'           => __( 'Legacy user level 1 (contributor). Deprecated — ignore.', 'tso-swiss-knife' ),
			'level_2'           => __( 'Legacy user level 2 (author). Deprecated — ignore.', 'tso-swiss-knife' ),
			'level_7'           => __( 'Legacy user level 7 (editor). Deprecated — ignore.', 'tso-swiss-knife' ),
			'level_10'          => __( 'Legacy user level 10 (administrator). Deprecated — ignore.', 'tso-swiss-knife' ),
			'manage_options'    => __( 'Access Settings and configure the site.', 'tso-swiss-knife' ),
			'edit_posts'        => __( 'Create and edit own posts.', 'tso-swiss-knife' ),
			'publish_posts'     => __( 'Publish posts.', 'tso-swiss-knife' ),
			'edit_others_posts' => __( 'Edit posts written by other users.', 'tso-swiss-knife' ),
			'delete_posts'      => __( 'Delete own posts.', 'tso-swiss-knife' ),
			'delete_others_posts' => __( 'Delete posts by other users.', 'tso-swiss-knife' ),
			'edit_pages'        => __( 'Edit pages.', 'tso-swiss-knife' ),
			'publish_pages'     => __( 'Publish pages.', 'tso-swiss-knife' ),
			'upload_files'      => __( 'Upload media to the library.', 'tso-swiss-knife' ),
			'moderate_comments' => __( 'Moderate comments.', 'tso-swiss-knife' ),
			'manage_categories' => __( 'Manage post categories.', 'tso-swiss-knife' ),
			'read'              => __( 'Read the dashboard (logged-in access).', 'tso-swiss-knife' ),
			'install_plugins'   => __( 'Install plugins — high risk.', 'tso-swiss-knife' ),
			'activate_plugins'  => __( 'Activate/deactivate plugins.', 'tso-swiss-knife' ),
			'edit_plugins'      => __( 'Edit plugin files — very high risk.', 'tso-swiss-knife' ),
			'edit_themes'       => __( 'Edit theme files — very high risk.', 'tso-swiss-knife' ),
			'unfiltered_html'   => __( 'Post HTML without filtering — XSS risk.', 'tso-swiss-knife' ),
			'edit_users'        => __( 'Edit other user accounts.', 'tso-swiss-knife' ),
		);
		if ( isset( $map[ $cap ] ) ) {
			return $map[ $cap ];
		}
		if ( 0 === strpos( $cap, 'level_' ) ) {
			return __( 'Legacy user level (deprecated). WordPress no longer uses levels for permissions.', 'tso-swiss-knife' );
		}
		if ( preg_match( '/^edit_(.+)$/', $cap, $m ) ) {
			return sprintf(
				/* translators: %s: post type slug */
				__( 'Edit content of type: %s', 'tso-swiss-knife' ),
				$m[1]
			);
		}
		if ( preg_match( '/^publish_(.+)$/', $cap, $m ) ) {
			return sprintf(
				/* translators: %s: post type slug */
				__( 'Publish content of type: %s', 'tso-swiss-knife' ),
				$m[1]
			);
		}
		if ( preg_match( '/^delete_(.+)$/', $cap, $m ) ) {
			return sprintf(
				/* translators: %s: post type slug */
				__( 'Delete content of type: %s', 'tso-swiss-knife' ),
				$m[1]
			);
		}
		return '';
	...
	public function ajax_clone(): void {
		...
		if ( ! $source || ! $new_id || ! $label ) {
			wp_send_json_error( __( 'Source role, new slug and label are required.', 'tso-swiss-knife' ) );
		}
		if ( 'administrator' === $source ) {
			wp_send_json_error( __( 'Cloning the administrator role is not allowed.', 'tso-swiss-knife' ) );
		}
		if ( get_role( $new_id ) ) {
			wp_send_json_error( __( 'A role with that slug already exists.', 'tso-swiss-knife' ) );
		}
		$caps = $this->get_role_caps( $source );
		if ( null === $caps ) {
			wp_send_json_error( __( 'Source role not found.', 'tso-swiss-knife' ) );
		}
		...
		TSOSK_Activity_Log::log(
			...
			sprintf(
				...
				__( 'Role cloned: %1$s → %2$s.', 'tso-swiss-knife' ),
				$source,
				$new_id
			)
		);
		wp_send_json_success( __( 'Role cloned. Reload the page to see it in the dropdown.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_apply_template(): void {
		...
		if ( 'administrator' === $role_id ) {
			wp_send_json_error( __( 'The administrator role cannot be modified from here.', 'tso-swiss-knife' ) );
		}
		if ( ! isset( $templates[ $template ] ) ) {
			wp_send_json_error( __( 'Unknown template.', 'tso-swiss-knife' ) );
		}
		$role = get_role( $role_id );
		if ( ! $role ) {
			wp_send_json_error( __( 'Role not found.', 'tso-swiss-knife' ) );
		...
		TSOSK_Activity_Log::log(
			...
			sprintf(
				...
				__( 'Template %2$s applied to role %1$s.', 'tso-swiss-knife' ),
				$role_id,
				$template
			)
		);
		wp_send_json_success( __( 'Template applied.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_add_cap(): void {
		...
		if ( ! $role_id || ! $cap ) {
			wp_send_json_error( __( 'Role and capability are required.', 'tso-swiss-knife' ) );
		}
		if ( 'administrator' === $role_id ) {
			wp_send_json_error( __( 'The administrator role cannot be modified from here.', 'tso-swiss-knife' ) );
		}
		if ( in_array( $cap, self::get_blocked_caps(), true ) ) {
			wp_send_json_error( __( 'This capability is blocked for safety.', 'tso-swiss-knife' ) );
		}
		...
		TSOSK_Activity_Log::log(
			...
			sprintf(
				...
				__( 'Capability added to %1$s: %2$s.', 'tso-swiss-knife' ),
				$role_id,
				$cap
			),
			array(
		...
		wp_send_json_success( __( 'Capability added.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_remove_cap(): void {
		...
		TSOSK_Activity_Log::log(
			...
			sprintf(
				/* translators: 1: role, 2: capability */
				__( 'Capability removed from %1$s: %2$s.', 'tso-swiss-knife' ),
				$role_id,
				$cap
			),
			array(
		...
		wp_send_json_success( __( 'Capability removed.', 'tso-swiss-knife' ) );
	}
	...
	public function render(): void {
		...
		?>
		<p class="tsosk-desc">
			<?php esc_html_e( 'Inspect, compare, clone and edit WordPress role capabilities. Dangerous capabilities are blocked or flagged.', 'tso-swiss-knife' ); ?>
		</p>

		<div class="tsosk-toolbar">
			<label>
				<?php esc_html_e( 'Role', 'tso-swiss-knife' ); ?>
				<select id="tsosk-roles-select">
					...
					<option value="<?php echo esc_attr( $id ); ?>" <?php selected( $id, $default_role ); ?>>
						...
						if ( 'administrator' === $id ) {
							$option_label .= ' — ' . __( 'read-only', 'tso-swiss-knife' );
						}
			...
			<button type="button" class="button" id="tsosk-roles-load"><?php esc_html_e( 'Load capabilities', 'tso-swiss-knife' ); ?></button>
			<span class="tsosk-badge tsosk-badge-info" id="tsosk-roles-count"></span>
		</div>

		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Compare roles', 'tso-swiss-knife' ); ?></h3>
			...
			<button type="button" class="button" id="tsosk-roles-compare"><?php esc_html_e( 'Compare', 'tso-swiss-knife' ); ?></button>
			<pre id="tsosk-roles-compare-out" class="tsosk-code" style="margin-top:10px;white-space:pre-wrap;display:none;"></pre>
		</div>

		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Clone role', 'tso-swiss-knife' ); ?></h3>
			<p class="description"><?php esc_html_e( 'Create a new role with the same capabilities as an existing one.', 'tso-swiss-knife' ); ?></p>
			<input type="text" id="tsosk-roles-clone-slug" class="regular-text" placeholder="<?php esc_attr_e( 'new_role_slug', 'tso-swiss-knife' ); ?>">
			<input type="text" id="tsosk-roles-clone-label" class="regular-text" placeholder="<?php esc_attr_e( 'Display name', 'tso-swiss-knife' ); ?>">
			<button type="button" class="button" id="tsosk-roles-clone"><?php esc_html_e( 'Clone selected role', 'tso-swiss-knife' ); ?></button>
		</div>

		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Apply template', 'tso-swiss-knife' ); ?></h3>
			<select id="tsosk-roles-template">
				<option value="content_editor"><?php esc_html_e( 'Content editor (posts + pages + media)', 'tso-swiss-knife' ); ?></option>
				<option value="media_manager"><?php esc_html_e( 'Media manager', 'tso-swiss-knife' ); ?></option>
				<option value="read_only"><?php esc_html_e( 'Read only', 'tso-swiss-knife' ); ?></option>
			</select>
			<p class="description"><?php esc_html_e( 'Replaces the role capabilities with the template (caps not in the template are removed; blocked caps are never granted).', 'tso-swiss-knife' ); ?></p>
			<button type="button" class="button" id="tsosk-roles-apply-template"><?php esc_html_e( 'Apply to selected role', 'tso-swiss-knife' ); ?></button>
		</div>

		<div class="tsosk-card" id="tsosk-roles-add-box">
			<h3><?php esc_html_e( 'Add capability', 'tso-swiss-knife' ); ?></h3>
			<p class="description"><?php esc_html_e( 'Only for custom roles (not administrator). Use lowercase with underscores, e.g. edit_products.', 'tso-swiss-knife' ); ?></p>
			<input type="text" id="tsosk-roles-new-cap" class="regular-text" placeholder="capability_name">
			<button type="button" class="button" id="tsosk-roles-add-cap"><?php esc_html_e( 'Add', 'tso-swiss-knife' ); ?></button>
		</div>

		<div class="tsosk-table-wrap">
			<p class="description"><?php esc_html_e( 'View-only for the administrator role. Capabilities cannot be added or removed here.', 'tso-swiss-knife' ); ?></p>
			<table class="widefat tsosk-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'Capability', 'tso-swiss-knife' ); ?></th>
						<th><?php esc_html_e( 'Description', 'tso-swiss-knife' ); ?></th>
						<th><?php esc_html_e( 'Actions', 'tso-swiss-knife' ); ?></th>
					</tr>
				</thead>
				<tbody id="tsosk-roles-tbody">
					<tr><td colspan="3"><?php esc_html_e( 'Select a role and click Load capabilities.', 'tso-swiss-knife' ); ?></td></tr>
				</tbody>
			</table>
		</div>
