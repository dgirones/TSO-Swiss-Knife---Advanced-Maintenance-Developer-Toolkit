...
class TSOSK_Mod_Debug {
	...
	public function ajax_wpconfig_save(): void {
		...
		if ( ! array_key_exists( $constant, $this->managed_constants() ) ) {
			wp_send_json_error( __( 'Unknown constant.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		if ( ! in_array( $value, array( 'true', 'false' ), true ) ) {
			wp_send_json_error( __( 'Invalid value. Only true or false are accepted.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}

		$path = $this->wpconfig_path();
		if ( '' === $path ) {
			wp_send_json_error( __( 'wp-config.php not found.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		if ( ! wp_is_writable( $path ) ) {
			wp_send_json_error( __( 'wp-config.php is not writable. Change file permissions to 644 temporarily, save, then restore to 440 or 400.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}

		// phpcs:ignore WordPress.WP.AlternativeFunctions.file_file -- reading local config file.
		$lines = file( $path );
		if ( ! is_array( $lines ) ) {
			wp_send_json_error( __( 'Could not read wp-config.php.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
...
";
		...
		if ( 0 !== strpos( ltrim( $new_content ), '<?php' ) ) {
			wp_send_json_error( __( 'Safety check failed: modified content does not start with <?php. Changes were NOT saved.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		...
		if ( false === $result ) {
			wp_send_json_error( __( 'Could not write wp-config.php.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		...
		TSOSK_Activity_Log::log(
			...
			sprintf(
				...
				__( 'wp-config.php: %1$s set to %2$s.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ),
				$constant,
				$value
			),
		...
		wp_send_json_success( array(
			'message'  => sprintf(
				/* translators: 1: constant name, 2: value */
				__( '%1$s set to %2$s in wp-config.php. Reload the page to see the new actual value.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ),
				$constant,
				$value
			),
			'constant' => $constant,
	...
	public function ajax_clear_log(): void {
		...
		$log = $this->validate_log_path( $posted_log );
		if ( '' === $log ) {
			wp_send_json_error( __( 'Invalid log file.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		if ( ! file_exists( $log ) ) {
	...
	public function ajax_shrink_log(): void {
		...
		$log        = $this->validate_log_path( $posted_log );
		if ( '' === $log || ! file_exists( $log ) || ! is_readable( $log ) ) {
			wp_send_json_error( __( 'Log file not found or not readable.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
		if ( ! wp_is_writable( $log ) ) {
			wp_send_json_error( __( 'Log file is not writable.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ) );
		}
...
<?php if ( $legacy_exists ) : ?>
		...
		<div class="tsosk-card">
			...
			<p class="description">
				<?php esc_html_e( 'Current values for each constant. Use the buttons to add or change them directly in wp-config.php when you need full control.', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ); ?>
			</p>
			...
			<div class="tsosk-debug-flags" id="tsosk-debug-form">
				...
				<div class="tsosk-flag-row<?php echo $wc_mismatch ? ' tsosk-flag-mismatch' : ''; ?>">
					...
					<div style="display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap;">
						...
						<button type="button"
							...
							if ( $in_wpconfig ) {
								...
								printf(
									...
									esc_html__( '%1$s Set to %2$s in wp-config.php', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ),
								...
								printf(
									...
									esc_html__( '+ Add %1$s = %2$s to wp-config.php', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ),
						...
						<span class="description" style="font-size:11px;">
							<?php esc_html_e( '(wp-config.php is read-only)', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ); ?>
						</span>
						<?php endif; ?>
					</div>
					<?php endif; ?>
				...
				<details class="tsosk-debug-advanced">
					<summary><?php esc_html_e( 'Advanced: write debug flags to wp-config.php', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ); ?></summary>
					<p class="description">
		...
		<div class="tsosk-card">
			...
			<div class="tsosk-notice tsosk-notice-info">
				...
				<?php esc_html_e( 'Created by PHP/your hosting server when PHP error logging is active. This plugin can read and clear it, but cannot enable it (that requires server/php.ini configuration).', 'tso-swiss-knife-advanced-maintenance-developer-toolkit' ); ?>
			</div>
