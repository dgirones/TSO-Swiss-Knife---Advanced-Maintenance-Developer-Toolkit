...
class TSOSK_Mod_Login_Protect {
	...
	public function block_direct_login_access(): void {
		...
		wp_die(
			esc_html__( 'Page not found.', 'tso-swiss-knife' ),
			esc_html__( 'Not Found', 'tso-swiss-knife' ),
			array( 'response' => 404 )
		);
	}
	...
	private function get_lockout_error_for_ip( string $ip ): ?WP_Error {
		...
		return new WP_Error(
			...
			sprintf(
				...
				__( 'Too many failed login attempts from %1$s. Please try again in %2$d minute(s).', 'tso-swiss-knife' ),
				esc_html( $ip ),
				$minutes
			)
		);
	...
	public function block_forbidden_username( $user, string $username, string $password ) {
		...
		return new WP_Error(
			...
			__( 'Too many failed login attempts. Please try again later.', 'tso-swiss-knife' )
		);
	}
	...
	public function enforce_login_maintenance(): void {
		...
		wp_die(
			esc_html__( 'Login is temporarily restricted to trusted IP addresses.', 'tso-swiss-knife' ),
			esc_html__( 'Login maintenance', 'tso-swiss-knife' ),
			array( 'response' => 503 )
		);
	}
	...
	public function verify_email_2fa( $user ) {
		...
		if ( '' === $submitted ) {
			if ( ! $stored ) {
				...
				wp_mail(
					...
					sprintf(
						...
						__( '[%s] Login verification code', 'tso-swiss-knife' ),
						get_bloginfo( 'name' )
					),
					sprintf(
						/* translators: 1: code, 2: minutes */
						__( "Your login verification code is: %1\$s\n\nIt expires in %2\$d minutes.", 'tso-swiss-knife' ),
						$code,
						10
					)
				);
			...
			return new WP_Error(
				'tsosk_2fa_required',
				__( 'A verification code was sent to your email. Enter it below and try again.', 'tso-swiss-knife' )
			);
		}
		...
		if ( ! is_string( $stored ) || ! hash_equals( $stored, $submitted ) ) {
			return new WP_Error(
				'tsosk_2fa_invalid',
				__( 'Invalid or expired verification code.', 'tso-swiss-knife' )
			);
		}
	...
	public function render_2fa_field(): void {
		echo '<p><label for="tsosk_2fa_code">' . esc_html__( 'Email verification code', 'tso-swiss-knife' ) . '<br>';
	...
	public function filter_2fa_login_errors( string $errors ): string {
		if ( isset( $_GET['tsosk_2fa'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$errors .= '<p>' . esc_html__( 'Enter the verification code sent to your email.', 'tso-swiss-knife' ) . '</p>';
		}
		return $errors;
	}
	...
	public function check_role_ip_whitelist( $user ) {
		...
		return new WP_Error(
			...
			__( 'Login from this IP address is not allowed for your role.', 'tso-swiss-knife' )
		);
	}
	...
	private function maybe_send_mass_lockout_alert(): void {
		...
		wp_mail(
			...
			sprintf(
				/* translators: %s: site name */
				__( '[%s] Mass login lockout alert', 'tso-swiss-knife' ),
				get_bloginfo( 'name' )
			),
			sprintf(
				/* translators: 1: count, 2: minutes */
				__( "%1\$d IP lockouts occurred in the last %2\$d minutes. Review Login Protection in TSO Swiss Knife.", 'tso-swiss-knife' ),
				$recent,
	...
	public function ajax_save(): void {
		...
		if ( $new['custom_url'] && empty( $new['login_slug'] ) ) {
			wp_send_json_error( __( 'Please enter a custom login slug before enabling the custom URL.', 'tso-swiss-knife' ) );
		}
		...
		if ( $new['custom_url'] && in_array( $new['login_slug'], $reserved, true ) ) {
			wp_send_json_error(
				sprintf(
					...
					__( '"%s" is a reserved slug and cannot be used as the login URL.', 'tso-swiss-knife' ),
					$new['login_slug']
				)
			);
		}
		...
		TSOSK_Activity_Log::log( 'login-protect', 'save', __( 'Login protection settings saved.', 'tso-swiss-knife' ) );

		wp_send_json_success( __( 'Login protection settings saved.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_unlock(): void {
		...
		if ( $reset_attempts_only ) {
			if ( '' === $ip ) {
				wp_send_json_error( __( 'Invalid IP address.', 'tso-swiss-knife' ) );
			}
			$attempts = $this->get_all_attempts();
			if ( isset( $attempts[ $ip ] ) ) {
				unset( $attempts[ $ip ] );
			...
			TSOSK_Activity_Log::log(
				'login-protect',
				'reset',
				__( 'Login attempt counter cleared.', 'tso-swiss-knife' ),
				array( 'ip' => $ip )
			);
			wp_send_json_success( __( 'Attempt counter cleared.', 'tso-swiss-knife' ) );
		}
		...
		TSOSK_Activity_Log::log(
			'login-protect',
			'reset',
			__( 'IP unlocked.', 'tso-swiss-knife' ),
			array( 'ip' => $ip )
		);

		wp_send_json_success( __( 'IP unlocked.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_unlock_all(): void {
		...
		TSOSK_Activity_Log::log( 'login-protect', 'reset', __( 'All locked IPs unlocked.', 'tso-swiss-knife' ) );

		wp_send_json_success( __( 'All IPs unlocked.', 'tso-swiss-knife' ) );
	}
	...
	public function ajax_clear_log(): void {
		...
		TSOSK_Activity_Log::log( 'login-protect', 'delete', __( 'Login lockout log cleared.', 'tso-swiss-knife' ) );

		wp_send_json_success( __( 'Lockout log cleared.', 'tso-swiss-knife' ) );
	}
	...
	public function render(): void {
		...
		?>
		<p class="tsosk-desc">
			<?php esc_html_e( 'Protect the WordPress login page against brute-force attacks and unwanted access. Each feature can be enabled and configured independently.', 'tso-swiss-knife' ); ?>
		</p>
		...
		<div class="tsosk-notice tsosk-notice-warn">
			<strong>
				...
				printf(
					...
					esc_html__( '⚠ %d IP address(es) are currently locked out.', 'tso-swiss-knife' ),
					count( $active_lockouts )
				);
				?>
			</strong>
			<?php esc_html_e( 'See the Lockout Log section below.', 'tso-swiss-knife' ); ?>
		</div>
		<?php endif; ?>
		...
		<div class="tsosk-card">
			<h3>
				...
				<?php esc_html_e( 'Custom Login URL', 'tso-swiss-knife' ); ?>
				<span class="tsosk-badge <?php echo $s['custom_url'] ? 'tsosk-badge-ok' : ''; ?>"
				      style="margin-left:10px;font-size:12px;">
					<?php echo $s['custom_url'] ? esc_html__( 'Active', 'tso-swiss-knife' ) : esc_html__( 'Inactive', 'tso-swiss-knife' ); ?>
				</span>
			</h3>

			<div class="tsosk-notice tsosk-notice-info">
				<?php esc_html_e( 'Changes the login page URL from the default /wp-login.php to a secret address only you know. Direct requests to /wp-login.php or /wp-admin/ while logged out will return a 404 instead of the login form, making the page invisible to bots.', 'tso-swiss-knife' ); ?>
				<br><strong><?php esc_html_e( 'Important:', 'tso-swiss-knife' ); ?></strong>
				<?php esc_html_e( 'Save the new URL before enabling — if you lose it, you can disable this feature from the database or directly from this panel via a whitelisted IP.', 'tso-swiss-knife' ); ?>
			</div>

			<?php if ( $s['custom_url'] && ! $xmlrpc_off ) : ?>
			<div class="tsosk-notice tsosk-notice-warn">
				<strong><?php esc_html_e( 'XML-RPC is still enabled.', 'tso-swiss-knife' ); ?></strong>
				<?php
				printf(
					wp_kses(
						/* translators: %s: Security tab URL */
						__( 'The custom login URL does not block authentication via <code>xmlrpc.php</code>. Disable XML-RPC in the <a href="%s">Security</a> tab for stronger protection.', 'tso-swiss-knife' ),
						array(
							'a'    => array( 'href' => array() ),
							'code' => array(),
						)
			...
			<table class="tsosk-kv-table" style="width:100%;max-width:600px;">
				<tr>
					<th style="width:200px;"><?php esc_html_e( 'Enable custom login URL', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="custom_url" id="tsosk-lp-custom-url"
							       value="1" <?php checked( $s['custom_url'] ); ?>>
							<?php esc_html_e( 'Hide /wp-login.php and use the slug below instead', 'tso-swiss-knife' ); ?>
						</label>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Login slug', 'tso-swiss-knife' ); ?></th>
					<td>
						<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
							...
							<input type="text" name="login_slug" id="tsosk-lp-slug"
							       ...
							       placeholder="<?php esc_attr_e( 'e.g. enter-4892', 'tso-swiss-knife' ); ?>"
							...
							<button type="button" class="button button-small" id="tsosk-lp-generate">
								<?php esc_html_e( 'Generate', 'tso-swiss-knife' ); ?>
							</button>
						</div>
						<?php if ( $s['custom_url'] && $s['login_slug'] ) : ?>
						<p class="description" style="margin-top:6px;">
							<?php esc_html_e( 'Current login URL:', 'tso-swiss-knife' ); ?>
		...
		<div class="tsosk-card">
			<h3>
				...
				<?php esc_html_e( 'Brute-Force Protection', 'tso-swiss-knife' ); ?>
				<span class="tsosk-badge <?php echo $s['brute_force'] ? 'tsosk-badge-ok' : ''; ?>"
				      style="margin-left:10px;font-size:12px;">
					<?php echo $s['brute_force'] ? esc_html__( 'Active', 'tso-swiss-knife' ) : esc_html__( 'Inactive', 'tso-swiss-knife' ); ?>
				</span>
			</h3>

			<div class="tsosk-notice tsosk-notice-info">
				<?php esc_html_e( 'Counts consecutive failed login attempts per IP address. Once the threshold is reached, further login attempts from that IP are blocked for the configured duration. Successful logins reset the counter.', 'tso-swiss-knife' ); ?>
			</div>

			<table class="tsosk-kv-table" style="width:100%;max-width:600px;">
				<tr>
					<th style="width:200px;"><?php esc_html_e( 'Enable brute-force protection', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="brute_force" id="tsosk-lp-brute"
							       value="1" <?php checked( $s['brute_force'] ); ?>>
							<?php esc_html_e( 'Block IPs after repeated failed logins', 'tso-swiss-knife' ); ?>
						</label>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Block forbidden usernames', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							...
							<?php esc_html_e( 'Instantly lock out IPs that try to log in with admin, administrador or administrator', 'tso-swiss-knife' ); ?>
						</label>
						<p class="description" style="margin-top:6px;">
							<?php esc_html_e( 'Uses the lockout duration below. Whitelisted IPs are exempt. Works even when brute-force counting is disabled.', 'tso-swiss-knife' ); ?>
						</p>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Max attempts', 'tso-swiss-knife' ); ?></th>
					<td>
						...
						<span class="description">
							<?php esc_html_e( 'failed attempts before lockout (recommended: 5)', 'tso-swiss-knife' ); ?>
						</span>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Attempt window', 'tso-swiss-knife' ); ?></th>
					<td>
						...
						<span class="description">
							<?php esc_html_e( 'minutes in which the attempts are counted (recommended: 5)', 'tso-swiss-knife' ); ?>
						</span>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Lockout duration', 'tso-swiss-knife' ); ?></th>
					<td>
						...
						<span class="description">
							<?php esc_html_e( 'minutes the IP is blocked (recommended: 30)', 'tso-swiss-knife' ); ?>
						</span>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Email notification', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="notify_email" id="tsosk-lp-notify"
							       value="1" <?php checked( $s['notify_email'] ); ?>>
							<?php esc_html_e( 'Send email when an IP is locked out', 'tso-swiss-knife' ); ?>
						</label>
						<br>
		...
		<div class="tsosk-card">
			<h3>
				...
				<?php esc_html_e( 'IP Whitelist', 'tso-swiss-knife' ); ?>
			</h3>

			<div class="tsosk-notice tsosk-notice-info">
				<?php esc_html_e( 'Whitelisted IPs always have access to the login page regardless of the custom URL setting, and are never locked out by the brute-force protection. Add your office and home IPs here.', 'tso-swiss-knife' ); ?>
			</div>

			<table class="tsosk-kv-table" style="width:100%;max-width:600px;">
				<tr>
					<th style="width:200px;vertical-align:top;padding-top:10px;">
						<?php esc_html_e( 'Whitelisted IPs', 'tso-swiss-knife' ); ?>
					</th>
					<td>
						<textarea name="whitelist_ips" id="tsosk-lp-whitelist" rows="4"
						          style="width:100%;font-family:monospace;font-size:13px;"
						          placeholder="<?php esc_attr_e( 'One IP per line, e.g.:', 'tso-swiss-knife' ); ?>&#10;192.168.1.10&#10;85.125.44.72"
						><?php echo esc_textarea( $s['whitelist_ips'] ); ?></textarea>
						<p class="description">
							<?php esc_html_e( 'Your current IP:', 'tso-swiss-knife' ); ?>
							...
							<button type="button" class="button button-small" id="tsosk-lp-add-my-ip"
								...
								<?php esc_html_e( 'Add my IP', 'tso-swiss-knife' ); ?>
							</button>
							<?php else : ?>
							<span class="tsosk-badge tsosk-badge-ok" style="font-size:11px;">
								<?php esc_html_e( 'Whitelisted', 'tso-swiss-knife' ); ?>
							</span>
							<?php endif; ?>
						</p>
					</td>
		...
		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Login Maintenance & Advanced', 'tso-swiss-knife' ); ?></h3>
			<table class="tsosk-kv-table" style="width:100%;max-width:700px;">
				<tr>
					<th style="width:200px;vertical-align:top;"><?php esc_html_e( 'Login maintenance', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="login_maintenance" id="tsosk-lp-maintenance" value="1" <?php checked( $s['login_maintenance'] ); ?>>
							<?php esc_html_e( 'Block login page except for trusted IPs below', 'tso-swiss-knife' ); ?>
						</label>
						<textarea name="login_maintenance_ips" id="tsosk-lp-maintenance-ips" rows="3"
						          ...
						          placeholder="<?php esc_attr_e( 'One IP per line allowed during maintenance', 'tso-swiss-knife' ); ?>"><?php echo esc_textarea( $s['login_maintenance_ips'] ); ?></textarea>
					</td>
				</tr>
				<tr>
					<th style="vertical-align:top;"><?php esc_html_e( 'Email verification (2FA)', 'tso-swiss-knife' ); ?></th>
					<td>
						<label>
							<input type="checkbox" name="email_2fa" id="tsosk-lp-2fa" value="1" <?php checked( $s['email_2fa'] ); ?>>
							<?php esc_html_e( 'Send a one-time code by email after password validation', 'tso-swiss-knife' ); ?>
						</label>
						<p class="description"><?php esc_html_e( 'Apply to roles:', 'tso-swiss-knife' ); ?></p>
				...
				<tr>
					<th style="vertical-align:top;"><?php esc_html_e( 'IP whitelist by role', 'tso-swiss-knife' ); ?></th>
					<td>
						...
						<p class="description"><?php esc_html_e( 'One role per line: role_slug: ip1, ip2. Global whitelist IPs always bypass this rule.', 'tso-swiss-knife' ); ?></p>
					</td>
				</tr>
				<tr>
					<th><?php esc_html_e( 'Mass lockout alert', 'tso-swiss-knife' ); ?></th>
					<td>
						...
						<span class="description"><?php esc_html_e( 'lockouts in', 'tso-swiss-knife' ); ?></span>
						...
						<span class="description"><?php esc_html_e( 'minutes (0 = disabled)', 'tso-swiss-knife' ); ?></span>
					</td>
				</tr>
			</table>
		</div>
		...
		<div class="tsosk-card">
			<h3><?php esc_html_e( 'Live Attempt Counters', 'tso-swiss-knife' ); ?></h3>
			<p class="description">
				<?php esc_html_e( 'IPs that have failed login attempts within the active window but have not yet been locked out.', 'tso-swiss-knife' ); ?>
			</p>
			<table class="widefat tsosk-table">
				<thead><tr>
					<th><?php esc_html_e( 'IP Address', 'tso-swiss-knife' ); ?></th>
					<th><?php esc_html_e( 'Failed Attempts', 'tso-swiss-knife' ); ?></th>
					<th><?php esc_html_e( 'First Attempt', 'tso-swiss-knife' ); ?></th>
				...
				<tbody>
					...
					<tr class="<?php echo $is_mine ? 'tsosk-row-warn' : ''; ?>">
						<td class="tsosk-code">
							...
							<?php if ( $is_mine ) : ?>
								<span class="tsosk-badge tsosk-badge-warn" style="font-size:11px;"><?php esc_html_e( 'Your IP', 'tso-swiss-knife' ); ?></span>
							<?php endif; ?>
						</td>
						<td>
		...
		<div class="tsosk-card">
			<h3>
				<?php esc_html_e( 'Lockout Log', 'tso-swiss-knife' ); ?>
			...
			<p style="color:#646970;"><?php esc_html_e( 'No lockouts have been recorded yet.', 'tso-swiss-knife' ); ?></p>
			<?php else : ?>
			...
			<div class="tsosk-toolbar" style="gap:8px;margin-bottom:12px;">
				...
				<button class="button" id="tsosk-lp-unlock-all"
				        data-nonce="<?php echo esc_attr( $nonce ); ?>">
					<?php esc_html_e( 'Unlock All', 'tso-swiss-knife' ); ?>
				</button>
				<?php endif; ?>
			...
			<div class="tsosk-table-wrap">
				<table class="widefat tsosk-table" id="tsosk-lp-log-table">
					<thead><tr>
						...
						<th><?php esc_html_e( 'Attempts', 'tso-swiss-knife' ); ?></th>
						<th><?php esc_html_e( 'Locked At', 'tso-swiss-knife' ); ?></th>
						<th><?php esc_html_e( 'Unlocks At', 'tso-swiss-knife' ); ?></th>
						<th><?php esc_html_e( 'Actions', 'tso-swiss-knife' ); ?></th>
					</tr></thead>
					<tbody>
					<?php
					...
					<tr class="<?php echo $is_active ? 'tsosk-row-warn' : ''; ?>">
						<td>
							<?php if ( $is_active ) : ?>
								<span class="tsosk-badge tsosk-badge-warn"><?php esc_html_e( 'Locked', 'tso-swiss-knife' ); ?></span>
							<?php else : ?>
								<span class="tsosk-badge"><?php esc_html_e( 'Expired', 'tso-swiss-knife' ); ?></span>
							<?php endif; ?>
						</td>
						<td class="tsosk-code">
							...
							<?php if ( $is_active ) : ?>
								...
								<br><small style="color:#666;">
									...
									printf(
										...
										esc_html__( '%s left', 'tso-swiss-knife' ),
						...
						<td class="tsosk-actions">
							...
							<button class="button button-small tsosk-lp-unlock"
							        ...
							        data-nonce="<?php echo esc_attr( $nonce ); ?>">
								<?php esc_html_e( 'Unlock', 'tso-swiss-knife' ); ?>
							</button>
							<?php else : ?>
